...
function createHeatMap(timeRanges) {
  ...
  timeRanges.forEach((timeRange) => {
    ...
    for (
      let i = timeToSegmentNumber(start);
      i <= timeToSegmentNumber(end);
      i++
    ) {
...
function heatMapToGradient(heatMap, maxVal) {
  ...
  const getColor = (alpha) =>
    `rgba(136, 153, 238, ${Number(alpha).toPrecision(3)})`;
...
function App() {
  ...
  const labels = [...Array(8).keys()].map(
    (n) => String(n * 3).padStart(2, "0") + ":00"
  );
  ...
  const averageTotalDurationPerLine = Math.floor(
    columns.reduce((acc, elem) => acc + elem.totalDuration, 0) / columns.length
  );

  const heatMapGradient = heatMapToGradient(
    createHeatMap(dataLines.flat()),
    dataLines.length
  );
    ...
    <main id="main">
      ...
      >
        ...
        <div
          ...
          title={`Average: ${durationToDurationString(
            averageTotalDurationPerLine
          )}`}
        ></div>
